...
def _add_to_group_summary(summary, id, start_time, time_taken=None, is_case_pass=None):
    """Add to group summary.

    :param dict summary: group summary.
    :param str id: case id.
    ...
    """
    key = "{}_{}".format(id, utility.date2str(start_time, '%Y%m%d%H%M%S'))
...
def _add_to_case_summary(summary, id, time_taken, is_case_pass):
    """Add to case summary.
    ...
    if id not in summary:
        summary[id] = {AVERAGE: time_taken,
                       TOTAL_COUNT: 1,
                       FAIL_COUNT: 0 if is_case_pass else 1,
                       MIN_TIME: time_taken,
                       MAX_TIME: time_taken}
    else:
        summary[id][TOTAL_COUNT] += 1
        if not is_case_pass:
            summary[id][FAIL_COUNT] += 1
        summary[id][AVERAGE] = (summary[id][AVERAGE] + time_taken) / 2.0
        summary[id][MIN_TIME] = min(summary[id][MIN_TIME], time_taken)
        summary[id][MAX_TIME] = max(summary[id][MAX_TIME], time_taken)
...
def calc(file_paths, group_minutes=60):
    ...
    """
    if not file_paths:
        raise ValueError('Plese provide file path.')
    ...
    if not group_summary:
        ...
        print("\n\nCase Id,Fail Count,Total Count,Failure Rate,Minimum Time Taken,Maximum Time Taken,Average Time Taken")
        groups = []
        for id, value in case_summary.items():
            print("{},{},{},{},{},{},{}".format(
                id,
                value[FAIL_COUNT],
                value[TOTAL_COUNT],
