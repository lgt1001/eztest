...
from . import ini, mail, testcase, testmode, utility, analyze_report

__version__ = "1.0.5"
module_name = "eztest"
version = "{} v{}".format(module_name, __version__)
__all__ = ["ini", "stringbuilder", "testcase", "utility", "analyze_report"]
...
"need_authentication" is boolean, "username" and "password" are required if "need_authentication" is True.""")
    ...
    _add_argument(calc_group, long_option="--calc", short_option="-ca", dest="calc", nargs="+",
                  help="Report files to be calculated.")
...
def main(args=None):
    ...
    try:
        ...
        if args.target:
            ...
            analyze_report.analyze(args.calc, args.group_minutes)
        else:
